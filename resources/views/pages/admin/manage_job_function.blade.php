...
<div id="page-content-wrapper" style="flex-grow: 1; overflow-x: hidden;">
    ...
    <div style="padding: 20px;">
        <h1 style="margin-top: 20px;">Manage Job Function</h1>

        <!-- Job Level Entry Form -->
        <div class="card" style="margin-top: 20px; padding: 20px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
            <h3>Job function Entry</h3>
            <form action="{{ route('job_function_store') }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label for="jobfunctionName" class="form-label">Job function Name</label>
                    <input type="text" id="jobfunctionName" name="name" class="form-control" placeholder="Enter job function name" required>
                </div>
                <button type="submit" class="btn btn-primary">Add Job function</button>
            </form>
        </div>

        <!-- Job Level List -->
        <div class="card" style="margin-top: 30px; padding: 20px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
            <h3>Job function List</h3>
            <table class="table table-bordered table-striped" style="margin-top: 20px;">
                <thead class="table-dark">
                    <tr>
                        <th>#</th>
                        <th>Job function Name</th>
                        <th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <!-- Example rows (replace with dynamic data) -->
                    @foreach ($jobfunctions as $index => $jobfunction)
                        <tr>
                            <td>{{ $index + 1 }}</td>
