...
<div id="page-content-wrapper" style="flex-grow: 1; overflow-x: hidden;">
    ...
    <div style="padding: 20px;">
        <h1 style="margin-top: 20px;">Manage Job Type</h1>

        <!-- Job Level Entry Form -->
        <div class="card" style="margin-top: 20px; padding: 20px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
            <h3>Job type Entry</h3>
            <form action="{{ route('job_type_store') }}" method="POST">
                @csrf
                <div class="mb-3">
                    <label for="jobtypeName" class="form-label">Job type Name</label>
                    <input type="text" id="jobtypeName" name="name" class="form-control" placeholder="Enter job Type name" required>
                </div>
